...
namespace ARWNI2S.Collections
{
    ...
    public static class Utils
    {
            ...
            catch (Exception exc)
            {
                try
                {
                    if (logger != null)
                    {
                        ...
                        {
                            logger.LogWarning((int)TraceCode.NodeRuntime_Error_100325, exc, String.Format(Resources.NodeRuntime_Error_100325, e.GetType().FullName, caller ?? string.Empty));
                        }
                    }
                }
                catch (Exception)
        ...
        public static AggregateException Flatten(this ReflectionTypeLoadException rtle)
        {
            ...
            throw new AggregateException(Resources.Utils_Flatten_AggregateException_Message, all);
        }
        ...
        public static MethodInfo GetStaticMethodThroughReflection(string assemblyName, string className, string methodName, Type[] argumentTypes)
        {
            ...
            if (asm == null)
                throw new InvalidOperationException(string.Format(Resources.Utils_AssemblyNull_Format, assemblyName));

            var cl = asm.GetType(className);
            if (cl == null)
                throw new InvalidOperationException(string.Format(Resources.Utils_ClassNull_Format, className, assemblyName));
            ...
            if (method == null)
                throw new InvalidOperationException(string.Format(Resources.Utils_GetStaticMethodThroughReflection_MethodNull_Format, methodName, className, assemblyName));
        ...
        {
            var asm = Assembly.Load(new AssemblyName(assemblyName));
            if (asm == null) throw new InvalidOperationException(string.Format(Resources.Utils_AssemblyNull_Format, assemblyName));

            var cl = asm.GetType(className);
            if (cl == null) throw new InvalidOperationException(string.Format(Resources.Utils_ClassNull_Format, className, assemblyName));
